...
if (isset($_GET['updateapply'])) {
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        $conpass = $_POST['cuppassword'];

        if($pass != $conpass){
            $alerterror = "Please enter valid password in both section!!";
        }else{
            $hashpassword = password_hash($pass, PASSWORD_DEFAULT);
            $uppasssql = "UPDATE `admin` SET `a_password` = '$hashpassword'";
            $passres = mysqli_query($conn,$uppasssql);
            if($passres){
    ...
    <div id="layoutSidenav_content">
        <main>
            ...
            <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                ...
                <?php
                if(isset($_GET['upadmin'])){
                $admin_id = $_GET['upadmin'];

                $updatesql = "SELECT *FROM `admin` WHERE `a_id` = $admin_id";
                $updateresult = mysqli_query($conn, $updatesql);

                while ($adrow = mysqli_fetch_assoc($updateresult)) {
                    $admin_image = "data:image/jpg;base64," . base64_encode($adrow['a_photo']);
                    $adname  = $adrow['a_name'];
                }
                echo '
                <script>
                ...
                <button type="button" style="display:none"  id="clickme"  data-bs-toggle="modal" data-bs-target="#updatecat">
                Launch demo modal
                </button>                    
                ...
                <h5 class="modal-title" ><strong>Update Password (' . $adname . ')</strong></h5>
                <a href="admin.php" type="button" class="btn-close"></a>
                </div>
                <div class="modal-body">
                <form method="POST" action="admin.php?updateapply=' . $admin_id . '" class="customer-form" enctype="multipart/form-data">
                <div id="catphoto">
                <label for="updatecatimage">
                    <img src="' . $admin_image . '" class="rounded-circle" data-bs-toggle="tooltip" data-bs-placement="right" title="Change category Photo">
                </label>
                </div>
                <input type="text" maxlength="20" class="form-control my-1" id="uppassword"  name="uppassword" placeholder="Enter Password" required="required" />
                <input type="text" maxlength="20" class="form-control my-1" id="cuppassword"  name="cuppassword" placeholder="Confirm Password" required="required" />
               
                <button type="submit" class="btn btn-success my-3"><i class="bi bi-pencil-fill"></i>  Update</button>
                    <a href="admin.php" type="button" class="btn btn-danger my-3"><i class="bi bi-x-circle"></i>  Cancel</a>
                </form>
                </div>
                </div>
                            ...
                            <table id="datatablesSimple1">
                                ...
                                <tbody>
                                    ...
                                    while ($row3 = mysqli_fetch_assoc($result3)) {
                                        ...
                                        if ($adminmaster == "nayan1@gmail.com") {
                                            echo '<td><a href="admin.php?deladmin=' . $row3['a_id'] . '" type="button" class="btn btn-danger"><i class="bi bi-trash"></i> Delete</a>
                                        <a href="admin.php?upadmin=' . $row3['a_id'] . '" type="button" class="btn btn-success"><i class="bi bi-pencil-fill"></i> Change Password</a>
                                        </td>';
                                        }
                                        echo '</tr>';
                                    }
